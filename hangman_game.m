...
disp(['The game of Hangman is a one player game that requires ', ...
    ...
    'Good luck!'])
pause(1) % allow time for them to read the instructions


% Create a word bank
%wordBank = {'lab', 'neuroscience', 'brain', 'computer', 'programming'}; % or inpuit each time = they need to remember where to change

%randomNumber = randi(size(wordBank,2), 1); % Get a random index
